var g_bat_dau = "";
var g_ket_thuc = "";
const THONG_BAO = "Nhấn Enter để xác nhận, Esc để hủy";
...
function resetTables(){
	...
	$("#ds_datsan_chuathanhtoan").html(LOADING_ORDERS);
...
function veTableDatSan(data) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		if (thanh_toan == "0") {
			html += "<td><button class='btnThanhToan btn btn-thanhtoan' datsan_id='" + data[i].datsan_id + "'>Xác nhận thanh toán</button></td>";
			html += "<td><button class='btnXoaDatSan btn btn-xoa' datsan_id='" + data[i].datsan_id + "'>Hủy sân</button></td>";
		} else {
			html += "<td><button class='btnThanhToan btn btn-dathanhtoan' datsan_id='" + data[i].datsan_id + "'>Hủy thanh toán</button></td>";
			html += "<td><button disabled class='btnXoaDatSan btn btn-xoa' datsan_id='" + data[i].datsan_id + "'>Hủy sân</button></td>";
		}
...
function veTableDatSanKhach(data) {
	...
	html +="<link rel='stylesheet' type='text/css' href='css/common.css' />";
	html += "<table class='table'>";
	html += "<thead class='thead-listdatsan'><tr><th>#</th><th>Tên KH</th><th>SĐT</th><th>Sân</th><th>Bắt đầu</th><th>Kết thúc</th><th>Phút</th><th>Đơn giá (/phút)</th><th>Tiền</th><th>Đã thanh toán</th><th>Thanh toán</th><th>Hủy</th></tr></thead>";
	html += "<tbody class='tbody-listdatsan'>";
	var tong_tien = 0;
	var da_thanh_toan = 0;
	var chua_thanh_toan = 0;
	...
	html += "<tr><td></td><td colspan=2 style='text-align:right; font-style:italic; color: red'><b>Số tài khoản:</b></td><td style='color: red; font-style: italic'>101003565630</td><td style='color: red; font-style: italic'><b>Vietinbank</b></td><td></td><td colspan=2 class='ttoan-list'><b>Chưa thanh toán</b></td><td style='font-weight:bold;color:red;'>" + formatMoney(chua_thanh_toan) + "đ</td><td></td><td></td><td></td></tr>";
	html += "<tr><td></td><td colspan=3 style='text-align:right; font-style:italic; color: red'><b>Nội dung:</b> '{Tên tài khoản} {Mã sân}'</td><td colspan=2 style='text-align:left; font-style:italic; color: red'> <b>(VD: tienanh 276)</b> </td><td colspan=2 class='ttoan-list'><b>Tổng tiền</b></td><td style='font-weight:bold;color:blue;'>" + formatMoney(tong_tien) + "đ</td><td></td><td></td><td></td></tr>";
	html += "</tfoot>";
	html += "</table>";
...
function veTableLichsu(data) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		if (thanh_toan == "0") {
			...
			html += "<td><button disabled class='btnThanhToan btn btn-dathanhtoan' datsan_id='" + data[i].datsan_id + "'>Đã thanh toán</button></td>";
			html += "<td><button disabled class='btnXoaDatSan btn btn-xoa' datsan_id='" + data[i].datsan_id + "'>Hủy sân</button></td>";
		}
...
function veTableChuathanhtoan(data) {
	var html = "";
	html +="<link href='https://cdn.jsdelivr.net/npm/bootstrap@5.2.0-beta1/dist/css/bootstrap.min.css' rel='stylesheet' integrity='sha384-0evHe/X+R7YkIZDRvuzKMRqM+OrBnVFBL6DOitfPri4tjfHxaWutUpFmBp4vmVor' crossorigin='anonymous'>";
	html +="<link rel='stylesheet' type='text/css' href='css/common.css' />";
	html += "<table class='table'>";
	...
	// if(thanh_toan==0){
	// 	html += "<tfoot class='tfood-listdatsan'>";
	// 	html += "<tr><td></td><td></td><td></td><td></td><td></td><td></td><td colspan=2 class='ttoan-list'><b>Tổng</b></td><td style='font-weight:bold;color:red;'>" + formatMoney(chua_thanh_toan) + "đ</td><td></td><td></td><td></td></tr>";
	// 	html += "</tfoot>";
	// }
	...
	html += "</table>";
	$("#ds_datsan_chuathanhtoan").html(html);
...
function xemDsChuathanhtoan(day) {
	resetTables();
	$.ajax({
		url: "/quanlysanbong/api/listchuathanhtoan.php",
		type: "GET",
		cache: false,
		data: {
			action: "listchuathanhtoan",
			thanhtoan: day
		},
		success: function(json) {
			var data = $.parseJSON(json);
